...
def _layout(fig, title=None, height=420):
    fig.update_layout(
        title=dict(text=title, font=dict(size=18, color=DARK)) if title else None,
        template="plotly_white",
        height=height,
        margin=dict(l=60, r=30, t=60 if title else 30, b=50),
        font=dict(family="Frutiger, Arial, sans-serif", size=13, color="#212B32"),
...
def _covid_band(fig):
    fig.add_vrect(x0=COVID_START, x1=COVID_END, fillcolor=GREY, opacity=0.12,
...
def national_performance_chart():
    df = analysis.national_performance("2015-01-01")
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df["period"], y=df["pct_within_4hrs"], mode="lines",
        name="All A&E types", line=dict(color=BLUE, width=2.5)))
...
def attendance_chart():
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=DARK, width=2), fill="tozeroy", fillcolor="rgba(0,94,184,0.08)"))
    _covid_band(fig)
...
def seasonal_index_chart():
    ...
    fig = go.Figure(go.Bar(x=df["month_name"], y=df["index"], marker_color=colors,
...
def trust_chart(org_name):
    ...
    fig.add_trace(go.Scatter(x=df["period"], y=df["pct_within_4hrs_t1"], mode="lines",
                             name=org_name, line=dict(color=BLUE, width=2.5)))
...
def trust_attendance_chart(org_name):
    df = analysis.trust_timeseries(org_name)
    fig = go.Figure(go.Bar(x=df["period"], y=df["att_type1"], marker_color=DARK,
...
def worst_trusts_chart():
    ...
    fig = go.Figure(go.Bar(
        ...
        marker_color=RED, text=df["avg_performance"].round(1).astype(str) + "%",
...
def rtt_trend_chart(specialties=None):
    ...
    for i, (name, g) in enumerate(df.groupby("treatment_function")):
        fig.add_trace(go.Scatter(x=g["period"], y=g["pct_over_18wk"], mode="lines",
...
def rtt_national_chart():
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df["period"], y=df["total_waiting"], name="Total waiting list",
                         marker_color="rgba(0,94,184,0.35)", yaxis="y"))
    fig.add_trace(go.Scatter(x=df["period"], y=df["pct_within_18wk"], name="% within 18 wks",
